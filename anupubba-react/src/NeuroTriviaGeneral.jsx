// src/components/NeuroTriviaGeneral.jsx
import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { db } from '../firebaseConfig';
import { collection, getDocs } from 'firebase/firestore';

function NeuroTriviaGeneral() {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const querySnapshot = await getDocs(collection(db, 'neuroTrivia'));
        const data = [];
  ...
  if (showResult) {
    ...
    let emoji = '';
    if (percentage >= 80) {
      message = '¡Excelente! Eres un experto en neurociencias.';
      emoji = '🧠';
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '600px', margin: '0 auto' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
        <h2 style={{ margin: 0 }}>🧠 Trivia de Neurociencias</h2>
...
export default NeuroTriviaGeneral;